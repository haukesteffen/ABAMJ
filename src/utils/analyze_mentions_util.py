...
def analyze_mentions(input_df):
    pivot_df = pd.pivot_table(input_df, index='medium', columns='party', values='title', aggfunc='count', fill_value=0).reindex(media)[parties]
    standardized_df = pivot_df.div(pivot_df.sum(axis=1), axis=0)
...
def saveplot(df, title, path):
    ...
    ax.set_xticklabels(['Linke', 'Grüne', 'SPD', 'FDP', 'CDU', 'CSU', 'AfD'], size=12, color='black')
    ax.set_yticklabels(['NDS', 'taz', 'Sp', 'ARD', 'ZDF', 'BR', 'ntv', 'faz', 'WELT', 'BILD', 'CTV'], size=12, color='black')
